# more - 歩くと応援してくれる運動アプリ

## 作品概要

**作品名：** more
**理由：** 「もっと(more)歩きたくなる」体験を届けたい、という思いから。<!-- TODO: 正式な由来があれば書き換え -->
...
| @chihiro12-t | バックエンド（音声キャッシュ） |
| <!-- TODO --> | ボイス（セリフ・事前生成・再生まわり） |
...
- **スマホブラウザの制約と正面から戦った** — iOS の許可ダイアログはユーザー操作の文脈でしか出せない、マナーモードで Web Audio が消音される、画面ロックでセンサーが止まる、などの罠を一つずつ回避
...
特に**依存を追加したら `docker compose up -d --build` でイメージを焼き直して go.mod / package.json をコミットする**こと（やらないと他のメンバーの環境が壊れる）。
...
- **事前準備：** 「起動するだけの骨組み」を用意（ビジネスロジック無しの状態からスタート）
- **1日目：** 歩数検出の実装と実機チューニング（二重カウント・ポケット誤検出の解析と対策）、タイトル画面、VOICEVOX クライアント
- **2日目：** 応援セリフ API・画面つなぎ込み、キャラ選択、音声キャッシュ（合成6秒→0.003秒）、経路マップ、iOS対応
- **3日目：** マイルストーン音声、目標設定の拡充（カロリー・体重）、UI仕上げ、デモ準備
